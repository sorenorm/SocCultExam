...
rounds = 1e2 # Number of rounds for updating the society
learning.rate = 0.8 # How quickly does the societies learn the optimal solution?
rfactor.mean = 0 # Mean of random factor that enables evolution
rfactor.sd = 0.05 # SD of random factor
comparison = 0.50 # The top 1-x % compared with the less optimal society
...
for(i in 4:(rounds + 4)){
  ...
  for (j in 1:nrow(rp)) {
    ...
    if (mean(world[[rp[j, 1]]]$Productivity) < mean(world[[rp[j, 2]]]$Productivity)) {
      Happiness.W1 <-
        Happiness.W1 +
        (sample(sort(Happiness.W1)[(population.size * comparison + 1):population.size], 1000, replace = T) -
           Happiness.W1) * learning.rate
    }
