...
for(i in rounds){
  ...
  for(k in 1:(n_agents/2)){
    ...
    perc1 <-
      (1 - agent1$materialism[1]) * (agent1$swb[i] - agent2$swb[i]) +
      agent1$materialism[1] *
      (0.5 * ((agent1$wealth[i] - agent2$wealth[i])/mean(agent1$wealth[i] + agent2$wealth[i])) +
        0.5 * ((a1_n_pp - a2_n_pp)/mean(a1_n_pp + a2_n_pp)))

    perc2 <-
      (1 - agent2$materialism[1]) * (agent2$swb[i] - agent1$swb[i]) +
      agent2$materialism[1] *
...
d %>% ggplot(aes(x = income, y = swb - 0.5 * set_level - 0.1 * circumstance)) +
  geom_point(alpha = 0.2, shape = 1) +
  geom_smooth(method = 'lm')

d %>% ggplot(aes(x = income, y = swb)) +
  geom_point(alpha = 0.2, shape = 1) +
  geom_smooth(method = 'lm')
